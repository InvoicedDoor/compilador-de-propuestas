export const getInfo = async (endpoint: string, token: string) => {
    const res = await fetch(`http://backend:5000/api/${endpoint}`, {
        headers: {
...
export const modifyInfo = async (endpoint: string, token: string, data: any, elementId: Number) => {
    const res = await fetch(`http://backend:5000/api/${endpoint}/${elementId}`, {
        headers: {
            "Authorization": `Bearer ${token}`,
        },
        method: "PATCH",
...
export const auth = async (data: any) => {
    const res = await fetch(`http://backend:5000/api/auth`, {
        method: "POST",
        headers: {
